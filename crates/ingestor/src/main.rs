...
impl RolloverStore {
    ...
    fn check_rollover(&mut self) -> Result<()> {
        ...
        if self.current_date != today {
            println!("Rolling over memory file from {} to {}", self.current_date, today);
    ...
    fn ingest(&mut self, data: &[u8], embedding: Vec<f32>, source_uri: &str) -> Result<()> {
        ...
        self.memvid.put_with_embedding_and_options(data, embedding, opts)?;
        self.counter += 1;
...
fn main() -> Result<()> {
    ...
    loop {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            let embedding: Vec<f32> = bincode::deserialize(&emb_bytes).context("Failed to deserialize embedding")?;
            
            let data = fs::read(&text_path).context("Failed to read text file")?;
            let uri = format!("mv2://ingest/{}", text_path.file_name().unwrap().to_string_lossy());
