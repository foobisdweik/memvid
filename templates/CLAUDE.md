...
your project name everywhere it appears (one find-and-replace).
-->
...
Write a full new shard via `memvid-write --project <project> --agent claude` at:
...
```bash
memvid-write --project <project> --agent claude <<'EOF'
<full new shard body>
EOF
```
